...
- 시스템 언어를 자동으로 따르며 중국어 간체·번체, 영어, 일본어, 한국어를 지원합니다. 설정 → 모양 → Language에서 즉시 전환할 수 있고 선택이 저장됩니다. 지원하지 않는 시스템 언어는 영어로 표시합니다.
...
4. 변경 사항은 자동 저장되며 헤더에 저장 상태가 표시됩니다. 기본 새로 고침 간격은 60초입니다.
